...
(async () => {
  try {
    ...
    const toniTokenContractAddress =
    ...
    toniTokenContract = new web3.eth.Contract(
      metadata.abi,
      toniTokenContractAddress
    );

    //** create the MasterChef **/

    //currentBlock = await web3.eth.getBlockNumber();
    currentBlock = 17306964;
    ...
    console.log(
      contractName,
      "Master Chef Contract deployed at address: ",
...
async function addLiquidityEth(
  ...
  const routerMetadata = JSON.parse(
    await remix.call("fileManager", "getFile", routerArtifactsPath)
  );
  const erc20Metadata = JSON.parse(
    await remix.call("fileManager", "getFile", erc20ArtifactsPath)
  );
  ...
  await tokenAContract.methods
    .approve(plsxrouter.options.address, amountADesired)
    .send({ from: accounts[0] });
  await tokenBContract.methods
    .approve(plsxrouter.options.address, amountBDesired)
    .send({ from: accounts[0] });
