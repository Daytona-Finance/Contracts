...
contract ToniToken is ERC20("Daytona.Finance", "TONI") {
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            "TONI::delegateBySig: invalid signature"
        );
        require(
            nonce == nonces[signatory]++,
            "TONI::delegateBySig: invalid nonce"
        );
        require(now <= expiry, "TONI::delegateBySig: signature expired");
        ...
        returns (uint256)
    {
        require(
            ...
            "TONI::getPriorVotes: not yet determined"
        );
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(
            block.number,
            "TONI::_writeCheckpoint: block number exceeds 32 bits"
        );
...
// ProofToken with Governance.
contract ProofToken is ERC20("Proof Token", "PROOF") {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (MasterChef).
    function mint(address _to, uint256 _amount) public onlyOwner {
        _mint(_to, _amount);
        _moveDelegates(address(0), _delegates[_to], _amount);
    ...
    constructor(ToniToken _toni) public {
        toni = _toni;
    }

    // Safe toni transfer function, just in case if rounding error causes pool to not have enough TONIs.
    function safeToniTransfer(address _to, uint256 _amount) public onlyOwner {
...
interface IMigratorChef {
    // Perform LP token migration from legacy Swap to ToniSwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to Swap LP tokens.
    // ToniSwap must mint EXACTLY the same amount of ToniSwap LP tokens or
    // else something bad will happen. Traditional Swap does not
    // do that so be careful!
    function migrate(IERC20 token) external returns (IERC20);
}

// MasterChef is the master of Toni. He can make Toni and he is a fair guy.
//
...
contract MasterChef is Ownable {
    ...
    // Update the given pool's TONI allocation point. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        bool _withUpdate
    ...
    function pendingToni(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 toniReward = multiplier
                .mul(toniPerBlock)
                .mul(pool.allocPoint)
                .div(totalAllocPoint);
            accToniPerShare = accToniPerShare.add(
                toniReward.mul(1e12).div(lpSupply)
            );
        }
        return
            user.amount.mul(accToniPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 toniReward = multiplier
            ...
            .mul(pool.allocPoint)
            .div(totalAllocPoint);
        toni.mint(devaddr, toniReward.div(40));
        toni.mint(devaddr2, toniReward.div(40));
        toni.mint(address(proof), toniReward);
        pool.accToniPerShare = pool.accToniPerShare.add(
            toniReward.mul(1e12).div(lpSupply)
        );
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        require(_pid != 0, "deposit TONI by staking");
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        require(_pid != 0, "withdraw TONI by unstaking");

        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(user.amount >= _amount, "withdraw: not good");
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accToniPerShare).div(1e12).sub(
            user.rewardDebt
        );
        if (pending > 0) {
            safeToniTransfer(msg.sender, pending);
    ...
    // Stake TONI tokens to MasterChef
    function enterStaking(uint256 _amount) public {
        ...
        proof.mint(msg.sender, _amount);
        emit Deposit(msg.sender, 0, _amount);
    }

    // Withdraw TONI tokens from STAKING.
    ...
    function safeToniTransfer(address _to, uint256 _amount) internal {
        proof.safeToniTransfer(_to, _amount);
    }
